...
public static class MonoCecilExtensions
{
    ...
    public static GenericParameter Clone(this GenericParameter parameter, IGenericParameterProvider newOwner)
    {
        var gp = new GenericParameter(parameter.Name, newOwner)
        {
            Attributes = parameter.Attributes
        };
        
        gp.Constraints.AddRange(parameter.Constraints.Select(x =>
        {
            var attr = new GenericParameterConstraint(x.ConstraintType);
            attr.CustomAttributes.AddRange(x.CustomAttributes.Select(y => y.Clone()));
            return attr;
        }));
    ...
    public static MethodDefinition Clone(this MethodDefinition method)
    {
        ...
        //copy generic params
        clonedMethod.GenericParameters.AddRange(method.GenericParameters.Select(x => x.Clone(clonedMethod)));
